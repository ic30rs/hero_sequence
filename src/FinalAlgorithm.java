...
public class FinalAlgorithm {
    ...
    private List<Hero> seq;
    private int seqWave = 0;
    private int waveCount = 0;
    private long finalRemain = Long.MAX_VALUE;
    private int solCount = 0;
    public int complex = 4;


    public static void main(String[] args) {
        int mComplex = 4;
        if(args.length == 1){
            mComplex = Integer.parseInt(args[0]);
        }
        System.out.println("计算中...");
        FinalAlgorithm algorithm = new FinalAlgorithm(mComplex);
        algorithm.fightSequence();
    }



    public FinalAlgorithm(int complex){
    ...
    private void dfs(Stack<Long> enemyStack, List<Hero> heroList, List<Hero> ans){
    ...
    private void dfsInternal( List<Hero> heroList, List<Hero> useList, PriorityQueue<PossibleSolution> possibleList, int start,  long targetDmg){
        int minUse = 0;
    ...
    private Stack<Long> initEnemies(){
...
class PossibleSolution implements Comparable<PossibleSolution>{
    ...
    PossibleSolution(long _dmg, List<Hero> _heros){
        dmg = _dmg;
        heros = _heros;
    }
    ...
    public int compareTo(PossibleSolution o) {
        ...
        if(o.dmg == dmg){
            return o.heros.size() > heros.size() ? -1 : 1;
        }
        return -1;
    }
}
